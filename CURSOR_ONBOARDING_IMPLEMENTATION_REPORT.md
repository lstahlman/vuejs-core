...
| No prior Cursor agent package | No `.cursor/`, no `AGENTS.md`, no `.agents/` |
...
Largest corrections: GraphQL-based issue discovery (emoji/shortcode label tooling bugs), exact label-name inventory, deferred environment.json/Bugbot, and fork-vs-upstream tracker targeting.
...
| `.../scripts/discover-candidates.mjs` | Read-only script | Reliable label/issue queries + hard exclusions | `gh` GraphQL | Validated against live tracker |
...
This checkout classification on 2026-07-27: **Ready** for unit work. Task-specific browser/e2e/dts needs remain **Partially ready** until those capabilities are verified for the selected task. Setup recommendation path: `pnpm install` or Cursor Set up Environment. No `.cursor/environment.json` committed.
...
| Query strategy | GraphQL `label.issues` ordered by `UPDATED_AT`; independent linked-PR scan; exclude cross-repo PR refs |
| Ranking | Rubric after hard exclusions; ≤3 results; conditional when no invitation |
| Confidence | High on exclusions/ownership; medium on fallback suitability without maintainer invitation |
| Limitations | Board fields unused; suitability changes quickly; no invented candidates when access fails |

Live rejection examples verified:

- `#15096` — open PR `#15097`
- `#15113` — `:exclamation: p4-important` SSR
- `#15126` / `#15153` / `#14893` — open linked PRs
- `#12088` — open linked PR `#12704` (script)

Conditional example for revalidation (not endorsed): `#14910` (`scope: playground`, unassigned, no linked PR, no invitation).
...
| Existing Cursor config preserved | Pass | None existed; package added cleanly | — |
| Label inventory | Pass | 59 labels | — |
| Primary newcomer queries | Pass | 0 / 0 open | — |
| Discovery script | Pass after Int-variable fix + cross-repo PR filter | Script output | Rate limits possible |
| Linked-PR exclusion | Pass | `#15096`, `#15126`, etc. | Timeline completeness varies |
...
2. Subagent frontmatter includes verified `readonly: true` and `is_background: false`.
3. Skills explicitly pin tracker host `vuejs/core` for fork checkouts.
4. Deferred `.cursor/environment.json` and `.cursor/BUGBOT.md` as recommended.
5. Created `docs/` directory (did not previously exist) for the human guide.
6. Cloud-agent delivery may open a PR on the working fork to land the package; onboarding skills themselves still forbid unsolicited pushes/issue writes.
