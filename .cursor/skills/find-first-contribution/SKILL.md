...
7. For each promising issue, retrieve full body, comments, labels, assignees, milestone, and **independently** search linked PRs (`gh pr list --repo vuejs/core --state all --search "<n>"` and timeline refs).
8. Map likely package and validation using built-in search / Explore.
9. Apply hard exclusions (see reference). Score remaining with the rubric.
10. Return at most **three** recommended or conditional candidates. Include rejected examples to show judgment.
11. If none qualify, say so. Repository-discovered ideas must be labeled **requiring maintainer confirmation**.
