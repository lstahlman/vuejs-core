...
verified_at: 2026-07-28T00:53:00Z
verified_commit: b5f8518379b77c3b62a7a9d2b52f6c76cda09bd5
access_method: gh-graphql
label_inventory:
  complete: true
  count: 59
```
...
## Unlabeled / uncategorized issues

Empty `good first issue` / `help wanted` queues must not end discovery. After primary + labeled fallback pools, scan a broad page of open issues (script section `unlabeledOrUncategorized`):

- **Unlabeled:** triage-unknown. Missing labels are not a suitability signal. Inspect full body/comments; cap confidence; recommend only as `conditional-maintainer-confirmation` when scope/repro/ownership clear.
- **Outside known signals:** has labels, but none in primary/fallback lists. Not newcomer-endorsed; same hard exclusions apply.
- These survivors are eligible for agent review options even though they are not exact newcomer-label matches — always state that explicitly.
...
4. Broad open-issue page for unlabeled / outside-known-signals survivors.
5. For each candidate: full body, comments, labels, assignees, milestones, linked PRs, repro, package map, validation path.
6. Apply hard exclusions, then score (see skill rubric). Return ≤3.
7. Access fallbacks: public browser → manual URL/export. Do not invent candidates.
