...
This document codifies conventions for **agent-driven contributions** to Vue core. It is derived from established project practice in [`.github/contributing.md`](./.github/contributing.md), [`.github/commit-convention.md`](./.github/commit-convention.md), [`.github/maintenance.md`](./.github/maintenance.md), and from patterns enforced by ESLint, Prettier, Vitest, and git hooks.
...
Common scopes in this repo: package or subsystem names such as `runtime-core`, `compiler-core`, `reactivity`, `runtime-dom`, `server-renderer`, `compiler-sfc`, `hydration`, `types`, `ci`, `deps`.

- Imperative mood: "fix" not "fixed".
- Do not capitalize the subject; no trailing `.`.
- Reference issues in the PR title / commit when fixing: `(fix #1234)` or `close #1234` in the body/footer as appropriate.
- Pre-commit runs `lint-staged` (ESLint + Prettier) and `pnpm check`. Do not bypass hooks unless explicitly instructed.
...
Prefer `slice` over `substring` (deprecated in `packages/global.d.ts`).

Use `import type` / inline type imports. Prefer `@ts-expect-error` over `@ts-ignore`.
...
Use compile-time constants from `packages/global.d.ts`:

- `__DEV__`, `__TEST__`, `__BROWSER__`, `__SSR__`, `__COMPAT__`, …
- `__FEATURE_OPTIONS_API__`, `__FEATURE_SUSPENSE__`, etc.
...
- Keep feature-flagged optional subsystems behind `__FEATURE_*` branches.
...
- Tests that expect warnings must use `toHaveBeenWarned` / `toHaveBeenWarnedLast` / `toHaveBeenWarnedTimes`. Unexpected warnings fail the suite.

### API visibility

- Public exports go through package `src/index.ts` barrels.
- Mark non-public exports with `/** @internal */` (or `@private`) rather than "quietly" exporting them.
...
6. Snapshot updates alone are insufficient for compiler output; add explicit `expect(code).toMatch(...)` (or equivalent) assertions.
7. Do not add tests solely to chase coverage numbers; cover meaningful behavior.
8. Type surface changes need coverage under `packages-private/dts-test` (`pnpm test-dts`).
9. Do not leave focused/disabled Vitest tests (`vitest/no-focused-tests`, `vitest/no-disabled-tests`).
