...
The research specification matches the current checkout closely: same commit, version, workspace layout, contributor docs, and absence of prior Cursor artifacts. The MVP shape (one always-on rule, one scoped package/test rule, one read-only onboarding subagent, three skills, contributor guide, contribution-signal reference) is **accepted**.
...
4. **Subagent schema:** Current Cursor docs support `readonly: true` and `is_background: false`. Include both.
5. **Environment:** This checkout is **Ready** for unit work (Node 22.14.0, pnpm 11.13.0, deps installed, `pnpm test packages/shared --run` passed). No repo `.cursor/environment.json`; cloud env is db-backed. Defer committing environment config.
6. **Optional Bugbot / completeness rule:** Deferred — no demonstrated Bugbot install or repeated omission evidence.
7. **Tracker host:** Skills must query **`vuejs/core`** even when the local remote is a fork.
...
| `find-first-contribution` + signal reference | 59 labels; GFI/HW empty; GraphQL label.issues works | Accept + Modify | Need GraphQL script; revalidate labels each run | Add `scripts/discover-candidates.mjs` |
...
4. Hard negatives: `:exclamation: p4-important`, `:fire: p5-urgent`, `has PR`, `need discussion`, `need more info`, active assignee/claim, open/draft linked PR, unresolved semantics, RFC/feature without approval.
5. Retrieve candidates via GraphQL `label.issues(states:OPEN, orderBy:UPDATED_AT)` — not `gh issue list --label` for emoji/shortcode labels.
6. Independently search linked PRs even when `has PR` is absent.
7. Without invitation/newcomer label: status is **conditional-maintainer-confirmation**, never “easy first issue.”
8. Rank at most three; reject misleading labeled issues explicitly.

Live verification sample (2026-07-27, `gh` GraphQL):

- `good first issue` / `help wanted`: 0 open.
- `#15096`: open type regression but PR `#15097` open → reject.
- `#15113`: `p4-important` SSR → reject by default.
- `#15126`, `#15153`, `#14893`: open linked PRs → reject.
- `#14910`: unassigned, no linked PR, `scope: playground` only → **conditional** (no maintainer invitation).
...
| Historical issue analyzer / DB | Rejected | Out of scope |
| Hard-coded durable “good first issue” demo pick | Rejected | Tracker empty; honesty is a feature |

## Remaining Uncertainties

- Whether `good first issue` is intentionally unused or awaiting triage (label still exists).
- Public project-board field usage for suitability (not verified; GraphQL board access not required for MVP).
