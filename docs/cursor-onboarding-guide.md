...
Proven bounded smoke on 2026-07-27: `pnpm test packages/shared --run`.
...
1. Inventories all labels (59 as of 2026-07-27).
...
4. Rejects active PRs, assignees, high-priority/SSR/architecture-risk work, and unresolved discussions.
5. Ranks at most three candidates with explicit confidence.
...
3. Watch discovery: primary labels empty → fallback pools → explicit rejects (e.g. `#15096` has PR `#15097`; `#15113` is `p4-important` SSR).
4. If a live candidate survives, select it; otherwise say no safe recommendation and use a known-issue or historical learning path.
